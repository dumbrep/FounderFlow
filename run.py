...
def run_server_in_new_terminal(server_file):
    ...
    if "lead_gen" in server_file:
        python_command = "python -m servers.lead_gen.lead_gen_server"
    else:
        python_command = f'python "{server_path}"'
    
    # PowerShell command to activate venv and run the server
    # -NoExit keeps the window open after execution
    command = (
        f'powershell -NoExit -Command "'
        f'Write-Host \"Starting {server_name} Server...\" -ForegroundColor Green; '
    ...
    try:
        ...
        print(f"✓ Launched {server_name} Server in new terminal")
